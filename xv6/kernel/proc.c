...
sysinfo(uint64 info)
{
  ...
  // Count the number of processes and total memory used
  for (p = proc; p < &proc[NPROC]; p++) {
  ...
  aval_mem = avalmem();
  newinfo.nproc = nproc;
  newinfo.usedbytes = usedbytes;
  newinfo.harts = harts;
  newinfo.totalmem = total_mem;
  newinfo.avalmem = aval_mem;
  
  // Copy the results to user space
  if (myproc() == 0)
     printf("DOESNT WORK!"); 
  //printf("&INFO: %d\n", info);
  //printf("UINT64 INFO: %d\n", (uint64)&info);
  
  if (copyout(myproc()->pagetable, info, (char *)&newinfo, sizeof(struct sysinfo)) < 0)  
